...
struct thunk_code {
  ...
  struct adjust_params {
    ...
    */
    ...
    adjust_params(size_t argc, const argument_info *arginfos) {
      ...
      case 0:
        if (argc == 5) {
          alignment_stack1 *_alignment_stack =
              reinterpret_cast<alignment_stack1 *>(p);
          new (_alignment_stack) alignment_stack1(5);
          p += sizeof(alignment_stack1);
        } else if (argc == 6) {
          alignment_stack2 *_alignment_stack =
              reinterpret_cast<alignment_stack2 *>(p);
          new (_alignment_stack) alignment_stack2(5);
          p += sizeof(alignment_stack2);
        } else if (argc > 6) {
          alignment_stack *_alignment_stack =
              reinterpret_cast<alignment_stack *>(p);
    ...
    static size_t calc_size(size_t argc, const argument_info *arginfos,
      ...
      *push_param_to_stack = false;
      ...
      if (argc > 3) {
        n += 4;
        if (argc > 4)
          *push_param_to_stack = true;
        if (argc == 5)
          n += sizeof(alignment_stack1);
        else if (argc == 6)
  ...
  thunk_code(call_declare, call_declare, size_t argc,
             const argument_info *arginfos) {
    ...
    if (push_param_to_stack) {
      ...
      return_caller *_return_caller =
          reinterpret_cast<return_caller *>(_restore_pc + 1);
  ...
  static size_t calc_size(call_declare, call_declare, size_t argc,
                          const argument_info *arginfos) {
    bool push_param_to_stack = false;
...
void base_thunk::init_code(call_declare caller, call_declare callee,
                           size_t argc,
                           const argument_info *arginfos) throw(bad_call) {
  _thunk_size = thunk_code::calc_size(caller, callee, argc, arginfos);

#if defined(_WIN32)
  _code = (char *)VirtualAlloc(NULL, _thunk_size, MEM_COMMIT,
